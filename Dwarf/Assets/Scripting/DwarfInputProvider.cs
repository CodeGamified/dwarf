// Copyright CodeGamified 2025-2026
// MIT License — Dwarf
using UnityEngine;
using UnityEngine.InputSystem;

namespace Dwarf.Scripting
{
    /// <summary>
    /// Captures keyboard input for fortress control.
    /// Encodes as a single float readable by bytecode via GET_INPUT:
    ///   0=none, 1=scroll_left, 2=scroll_right, 3=scroll_up, 4=scroll_down,
    ///   5=zoom_in, 6=zoom_out, 7=dig_mode, 8=build_mode
    /// </summary>
    public class DwarfInputProvider : MonoBehaviour
    {
        public static DwarfInputProvider Instance { get; private set; }

        public const float INPUT_NONE       = 0f;
        public const float INPUT_LEFT       = 1f;
        public const float INPUT_RIGHT      = 2f;
        public const float INPUT_UP         = 3f;
        public const float INPUT_DOWN       = 4f;
        public const float INPUT_ZOOM_IN    = 5f;
        public const float INPUT_ZOOM_OUT   = 6f;
        public const float INPUT_DIG        = 7f;   // 'd' key — toggle dig mode
        public const float INPUT_BUILD      = 8f;   // 'b' key — toggle build mode

        public float CurrentInput { get; private set; }

        private InputAction _leftAction;
        private InputAction _rightAction;
        private InputAction _upAction;
        private InputAction _downAction;
        private InputAction _zoomInAction;
        private InputAction _zoomOutAction;
        private InputAction _digAction;
        private InputAction _buildAction;

        private void Awake()
        {
            Instance = this;

            _leftAction = new InputAction("Left", InputActionType.Button);
            _leftAction.AddBinding("<Keyboard>/leftArrow");
            _leftAction.AddBinding("<Keyboard>/a");
            _leftAction.Enable();

            _rightAction = new InputAction("Right", InputActionType.Button);
            _rightAction.AddBinding("<Keyboard>/rightArrow");
            _rightAction.AddBinding("<Keyboard>/d");
            _rightAction.Enable();

            _upAction = new InputAction("Up", InputActionType.Button);
            _upAction.AddBinding("<Keyboard>/upArrow");
            _upAction.AddBinding("<Keyboard>/w");
            _upAction.Enable();

            _downAction = new InputAction("Down", InputActionType.Button);
            _downAction.AddBinding("<Keyboard>/downArrow");
            _downAction.AddBinding("<Keyboard>/s");
            _downAction.Enable();

            _zoomInAction = new InputAction("ZoomIn", InputActionType.Button);
            _zoomInAction.AddBinding("<Keyboard>/equals");
            _zoomInAction.AddBinding("<Keyboard>/numpadPlus");
            _zoomInAction.Enable();

            _zoomOutAction = new InputAction("ZoomOut", InputActionType.Button);
            _zoomOutAction.AddBinding("<Keyboard>/minus");
            _zoomOutAction.AddBinding("<Keyboard>/numpadMinus");
            _zoomOutAction.Enable();

            _digAction = new InputAction("Dig", InputActionType.Button);
            _digAction.AddBinding("<Keyboard>/x");
            _digAction.Enable();

            _buildAction = new InputAction("Build", InputActionType.Button);
            _buildAction.AddBinding("<Keyboard>/b");
            _buildAction.Enable();
        }

        private void Update()
        {
            if (_leftAction.WasPressedThisFrame())
                CurrentInput = INPUT_LEFT;
            else if (_rightAction.WasPressedThisFrame())
                CurrentInput = INPUT_RIGHT;
            else if (_upAction.WasPressedThisFrame())
                CurrentInput = INPUT_UP;
            else if (_downAction.WasPressedThisFrame())
                CurrentInput = INPUT_DOWN;
            else if (_zoomInAction.WasPressedThisFrame())
                CurrentInput = INPUT_ZOOM_IN;
            else if (_zoomOutAction.WasPressedThisFrame())
                CurrentInput = INPUT_ZOOM_OUT;
            else if (_digAction.WasPressedThisFrame())
                CurrentInput = INPUT_DIG;
            else if (_buildAction.WasPressedThisFrame())
                CurrentInput = INPUT_BUILD;
            else
                CurrentInput = INPUT_NONE;
        }

        private void OnDestroy()
        {
            _leftAction?.Disable();    _leftAction?.Dispose();
            _rightAction?.Disable();   _rightAction?.Dispose();
            _upAction?.Disable();      _upAction?.Dispose();
            _downAction?.Disable();    _downAction?.Dispose();
            _zoomInAction?.Disable();  _zoomInAction?.Dispose();
            _zoomOutAction?.Disable(); _zoomOutAction?.Dispose();
            _digAction?.Disable();     _digAction?.Dispose();
            _buildAction?.Disable();   _buildAction?.Dispose();
            if (Instance == this) Instance = null;
        }
    }
}
